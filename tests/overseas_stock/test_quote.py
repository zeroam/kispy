...
def test_get_stock_price_history(auth: AuthAPI):
...
def test_get_stock_price_history_with_empty(auth: AuthAPI):
    quote = KisClient(auth).overseas_stock.quote
...
@freeze_time("2024-01-03")
def test_get_stock_price_history_with_end_date_none(auth: AuthAPI):
    quote = KisClient(auth).overseas_stock.quote
...
@freeze_time("2024-01-03")
def test_get_stock_price_history_with_end_date_future(auth: AuthAPI):
    quote = KisClient(auth).overseas_stock.quote
    today = datetime.now().strftime("%Y-%m-%d")
